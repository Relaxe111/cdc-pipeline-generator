...
def resolve_sink_env_key(
    ...
    if target_sink_env is not None:
        ...
        if requested and requested in source_cfg and isinstance(source_cfg.get(requested), dict):
            return requested
        raise ValueError(
            f"Invalid target_sink_env '{target_sink_env}'. "
            + f"Available sink envs: {available_envs}"
        )
...
def get_sink_target_env_keys(
    ...
    if not isinstance(sink_source_raw, dict):
        return None, (
            f"Sink source '{sink_source_name}' not found in sink group "
            + f"'{sink_group_name}'"
        )
    ...
    if not env_keys:
        return None, (
            f"Sink source '{sink_group_name}.{sink_source_name}' has no environment entries"
        )
