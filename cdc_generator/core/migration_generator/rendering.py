...
def _remove_stale_infrastructure_file(path: Path) -> None:
    """Remove infrastructure files that are incompatible with the current runtime."""
    if path.exists():
        path.unlink()
...
def generate_infrastructure(
    ...
    legacy_broker_sql_path = infra_dir / "02-cdc-management.sql"
    native_runtime_sql_path = infra_dir / "03-native-cdc-runtime.sql"

    if ctx.runtime_mode == "native":
        _remove_stale_infrastructure_file(legacy_broker_sql_path)
    else:
        _remove_stale_infrastructure_file(native_runtime_sql_path)
