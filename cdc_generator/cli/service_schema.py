...
Usage:
    ...
    cdc manage-service-schema --service chat \\
    ...
    cdc manage-service-schema --list-services
"""
...
def _build_parser() -> SchemaArgumentParser:
    """Build argument parser for manage-service-schema."""
...
def _handle_add_custom_table(
    ...
    if not column_specs:
        ...
        print_info(
            "Example: cdc manage-service-schema"
            + " --service chat"
            + " --add-custom-table public.audit_log"
            + " --column id:uuid:pk"
            + " --column event_type:text:not_null"
...
def main() -> int:
    """Entry point for ``cdc manage-service-schema``."""
