...
def handle_remove_sink_group_command(args: argparse.Namespace) -> int:
    ...
    if sink_group.get("inherits", False):
        ...
        print_error(f"Cannot remove '{sink_group_name}' — it inherits from" + f" source group '{source_name}'")
        print_info("Inherited sink groups are auto-generated by --create." + " Remove the source group instead, or recreate without it.")
        return 1
...
def handle_add_source_custom_key_command(args: argparse.Namespace) -> int:
    ...
    print_success(f"Saved source custom key '{key_name}' for sink group '{sink_group_name}'")
    return 0
...
def handle_add_to_ignore_list_command(args: argparse.Namespace) -> int:
    ...
    print_success(f"Updated database_exclude_patterns for sink group '{sink_group_name}'")
    print_info(f"Patterns: +{added_count} added" + f" (total: {len(merged_patterns)})")
    return 0


def handle_add_to_include_list_command(args: argparse.Namespace) -> int:
    """Add database include pattern(s) to sink group configuration."""
    if not args.add_to_include_list:
        print_error("--add-to-include-list requires a pattern")
        return 1

    parsed_patterns = _parse_csv_patterns(str(args.add_to_include_list))
    if not parsed_patterns:
        print_error("No valid patterns provided for --add-to-include-list")
        return 1

    result = resolve_sink_group_for_pattern_update(
        args,
        action_flag="--add-to-include-list",
    )
    if isinstance(result, int):
        return result
    ...
    sink_groups, sink_group_name, sink_group, sink_file = result
    merged_patterns, added_count = _append_unique_patterns(
        sink_group.get("database_include_patterns", []),
        parsed_patterns,
    )
    cast(dict[str, Any], sink_group)["database_include_patterns"] = merged_patterns
    sink_groups[sink_group_name] = sink_group
    save_sink_groups(sink_groups, sink_file)

    print_success(f"Updated database_include_patterns for sink group '{sink_group_name}'")
    print_info(f"Patterns: +{added_count} added" + f" (total: {len(merged_patterns)})")
    return 0


def handle_set_include_list_command(args: argparse.Namespace) -> int:
    """Replace database include pattern(s) on a sink group."""
    if not args.set_include_list:
        print_error("--set-include-list requires a pattern")
        return 1

    parsed_patterns = _parse_csv_patterns(str(args.set_include_list))
    if not parsed_patterns:
        print_error("No valid patterns provided for --set-include-list")
        return 1

    result = resolve_sink_group_for_pattern_update(
        args,
        action_flag="--set-include-list",
    )
    if isinstance(result, int):
        return result

    sink_groups, sink_group_name, sink_group, sink_file = result
    cast(dict[str, Any], sink_group)["database_include_patterns"] = parsed_patterns
    sink_groups[sink_group_name] = sink_group
    save_sink_groups(sink_groups, sink_file)

    print_success(f"Replaced database_include_patterns for sink group '{sink_group_name}'")
    print_info(f"Patterns: {parsed_patterns}")
    return 0
...
def handle_add_to_schema_excludes_command(args: argparse.Namespace) -> int:
    ...
    print_success(f"Updated schema_exclude_patterns for sink group '{sink_group_name}'")
    print_info(f"Patterns: +{added_count} added" + f" (total: {len(merged_patterns)})")
    return 0
...
def handle_add_to_table_excludes_command(args: argparse.Namespace) -> int:
    ...
    print_success(f"Updated table_exclude_patterns for sink group '{sink_group_name}'")
    print_info(f"Patterns: +{added_count} added" + f" (total: {len(merged_patterns)})")
    return 0
