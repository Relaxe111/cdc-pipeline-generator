...
def _apply_route_metadata(env_entry: dict[str, Any], db: dict[str, Any]) -> None:
    """Copy optional route metadata from discovered database rows into YAML entries."""
    target_sink_env_raw = db.get("target_sink_env")
    if isinstance(target_sink_env_raw, str) and target_sink_env_raw.strip():
        env_entry["target_sink_env"] = target_sink_env_raw.strip()


def build_db_shared_structure(databases: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """
    ...
    for db in databases:
        ...
        if service not in source_data:
            source_data[service] = {"schemas": set(), "environments": {}}
        ...
        if env:
            ...
            if isinstance(custom_values, dict):
                for key_raw, value_raw in cast(dict[object, object], custom_values).items():
...
def build_db_per_tenant_structure(databases: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """
...
def convert_to_yaml_structure(source_data: dict[str, dict[str, Any]], pattern: str) -> dict[str, Any]:
    """
    ...
    for source_name, data in sorted(source_data.items()):
        sources[source_name] = {"schemas": sorted(data["schemas"])}
