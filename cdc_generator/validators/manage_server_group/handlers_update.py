...
def _print_update_header(sg_name: str, sg_type: str | None, sg_pattern: str, include_pattern: str | None) -> None:
...
def _inspect_server_databases(  # noqa: PLR0913
    ...
    Example:
        ...
        ...     'default', config, group, 'postgres', None, [], []
        ... )
        >>> len(databases)
        5
    """
...
def _get_entry_str(entry: dict[str, object], key: str, default: str) -> str:
    value = entry.get(key)
    return value if isinstance(value, str) else default


def _get_entry_int(entry: dict[str, object], key: str, default: int) -> int:
    value = entry.get(key)
    return value if isinstance(value, int) else default


def _get_entry_list(entry: dict[str, object], key: str) -> list[str]:
    value = entry.get(key)
...
def _merge_with_existing_sources(
    ...
    for source_name, source_config in existing_sources.items():
        ...
        source_dict = cast(dict[str, object], source_config)
        schemas = _get_entry_list(source_dict, "schemas")
        ...
        for key, value in source_dict.items():
            ...
            env_entry = cast(dict[str, object], value)
            env_server = _get_entry_str(env_entry, "server", "default")

            # Only preserve if this server was NOT updated and we don't already have it
            if env_server not in updated_servers and (source_name, env) not in scanned_keys:
                # Preserve this database
                preserved_db: DatabaseInfo = {
                    "name": _get_entry_str(env_entry, "database", source_name),
                    "service": source_name,
                    "environment": env,
                    "server": env_server,
...
def _handle_connection_error(error: Exception, sg_name: str) -> int:
    ...
    if isinstance(error, MissingEnvironmentVariableError):
        ...
        print_info(
            "Export the missing variable in your shell (e.g. `set -x NAME value`) "
...
def handle_update(args: Namespace) -> int:
    ...
    try:
        while True:
            ...
            scan_failed = False
            source_custom_keys = normalize_source_custom_keys(server_group.get("source_custom_keys", {}))
            ...
            all_databases = _merge_with_existing_sources(server_group, scanned_databases, updated_server_names)
            ...
            print_success("\nTotal: " + f"{len(all_databases)} database(s) across " + f"{len(servers_to_update)} server(s)")
