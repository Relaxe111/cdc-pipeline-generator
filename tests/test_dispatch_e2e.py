"""End-to-end dispatch tests for CLI flags not covered by test_dispatch.py.
...
def project_dir(tmp_path: Path) -> Iterator[Path]:
    """Isolated project dir with services/, source-groups, sink-groups."""
...
class TestHandleValidateBloblang:
    """Direct handler tests for handle_validate_bloblang."""

    def test_returns_0_on_success(
        self, project_dir: Path, service_yaml: Path,
    ) -> None:
        """Returns 0 when validation succeeds."""
...
class TestHandleRemoveColumnTemplateHappyPath:
    """Happy-path tests for handle_remove_column_template."""
...
class TestHandleRemoveTransformHappyPath:
    """Happy-path tests for handle_remove_transform."""
...
class TestHandleListTransformsHappyPath:
    """Happy-path tests for handle_list_transforms."""

    def test_returns_0_with_data(
        self, project_dir: Path,
    ) -> None:
...
class TestHandleAddCustomSinkTableHappyPath:
    ...
    def test_returns_0_on_success(
        ...
        # Patch SERVICE_SCHEMAS_DIR at the import site so the
        # schema-file existence check sees the tmp_path version.
        with patch(
...
class TestHandleGenerateValidationHappyPath:
    """Happy-path tests for handle_generate_validation."""
    ...
    def test_returns_0_with_all(
        ...
        args = _full_ns(all=True, generate_validation=True)
        with patch(
    ...
    def test_returns_0_with_schema(
        ...
        args = _full_ns(schema="public", generate_validation=True)
        with patch(
            "cdc_generator.cli.service_handlers_validation.generate_service_validation_schema",
            return_value=True,
        ):
