...
exactly as a user would type in their terminal.  This validates
...
(e.g. in a bare CI environment), every test that depends on ``run_cdc``
is automatically skipped with a clear reason.  ``run_cdc_completion``
never requires fish because it works at the Python/Click level.
"""
...
_SKIP_REASON_FISH = "fish shell is not installed (required for CLI e2e tests)"
...
def run_cdc_completion() -> RunCdcCompletion:
    ...
    drives the runtime ``_CDC_COMPLETE`` protocol.
    ...
    def _complete(partial_cmd: str) -> subprocess.CompletedProcess[str]:
        ...
        lines = [f"{c.value}\t{c.help or ''}" if c.help else c.value for c in completions]
        stdout = "\n".join(lines) + "\n" if lines else ""
