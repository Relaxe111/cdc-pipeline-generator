# Manage Migrations — Status Audit

> **Temporary working document.** Will be refined and moved to `_docs/` when migration work is complete.
>
> Audit date: 2026-02-26
>
> **Implementation plan:** [MIGRATION_IMPLEMENTATION_PLAN.md](MIGRATION_IMPLEMENTATION_PLAN.md) — tracks phases, tasks, and progress for fixing all gaps identified here.
...
**Overall status: 🔴 Migration generation is non-functional.**

The old `scripts/2-apply-cdc-tables.py` script — which generated all `pg-migrations/*.sql` files — was **deleted** during the project restructuring but **never migrated** into `cdc_generator/`. The existing SQL files in `generated/pg-migrations/` are **stale artifacts** from the last manual run (2026-01-29).

| Aspect | Status | Details |
|--------|--------|---------|
| CLI commands registered | ⚠️ Partial | 4 subcommands in Click, but only 1 (`schema-docs`) routed in `MIGRATION_COMMANDS` |
| Migration SQL generation | 🔴 Missing | No code in `cdc_generator/` writes to `generated/pg-migrations/` |
| Table-definition generation | 🔴 Missing | No code regenerates `generated/table-definitions/*.yaml` from MSSQL |
| Service config awareness | 🔴 None | Generated migrations cover 3 tables; service config has 39 source tables |
| Versioning / schema evolution | 🔴 None | No version tracking, no diff, no incremental ALTER |
| Staging table generation | 🟡 Stale | SQL exists for 3 tables, but not auto-generated from current config |
| Flag handling (`target_exists`, `replicate_structure`) | 🔴 Not connected | Flags exist in service config & types but are not consumed by migration gen |
| Offset gap setup | 🟡 Partial | `cdc_processing_log` SQL exists, but not auto-generated per table |
| Target schema creation | 🟡 Stale | `1-create-customer-schemas.sql` exists but was manually generated |

---

## 2. Current CLI Surface

> **Plan:** [Phase 0 — CLI Cleanup](MIGRATION_IMPLEMENTATION_PLAN.md#phase-0-cli-cleanup)

### 2.1 Registered Subcommands

File: `cdc_generator/cli/click_commands.py` (lines 1334–1420)

| Subcommand | Click registered | In `MIGRATION_COMMANDS` dict | Actual module | Status |
|-----------|:--:|:--:|-----------|--------|
| `schema-docs` | ✅ | ✅ | `cdc_generator.cli.schema_docs` | ✅ Works — inspects MSSQL, outputs YAML to `generated/schemas/` |
| `enable-cdc` | ✅ | ❌ | *(none)* | 🔴 **Will fail** — "Unknown subcommand" at runtime |
| `apply-replica` | ✅ | ❌ | *(none)* | 🔴 **Will fail** — "Unknown subcommand" at runtime |
| `clean-cdc` | ✅ | ❌ | *(none)* | 🔴 **Will fail** — "Unknown subcommand" at runtime |

### 2.2 Routing Issue

`click_commands.py` registers Click subcommands that passthrough to `execute_grouped_command()`.
That function looks up `MIGRATION_COMMANDS` dict in `commands.py` — which only contains `schema-docs`.
...
> **Plan:** [Phase 1 — Core Migration Generator](MIGRATION_IMPLEMENTATION_PLAN.md#phase-1-core-migration-generator)

### 3.1 Current Files in `generated/pg-migrations/`

| File | Purpose | Generated by | Current state |
|------|---------|-------------|---------------|
| `0-cdc-merge-control.sql` | CDC merge control system (schema, tables, triggers, views) | Old `scripts/2-apply-cdc-tables.py` | ✅ Complete but static |
| `1-create-customer-schemas.sql` | `CREATE SCHEMA IF NOT EXISTS` for 26 customers | Old script | 🟡 Static — won't adapt to new customers |
| `2-cdc-processing-log.sql` | Offset tracking, gap detection, replication lag views | Old script | ✅ Complete but uses `{{SCHEMA}}` placeholder |
| `3-Actor.sql` | Final table DDL for Actor | Old script (MSSQL inspection) | ⚠️ One of only 3 tables generated |
| `3-Fraver.sql` | Final table DDL for Fraver | Old script | ⚠️ One of only 3 tables generated |
| `3-Test.sql` | Final table DDL for Test | Old script | ⚠️ Test table — may not be needed |
| `4-Actor-staging.sql` | UNLOGGED staging table + merge procedure for Actor | Old script | ⚠️ Complex, correct but static |
| `4-Fraver-staging.sql` | UNLOGGED staging table + merge procedure for Fraver | Old script | ⚠️ Complex, correct but static |
| `4-Test-staging.sql` | UNLOGGED staging table + merge procedure for Test | Old script | ⚠️ May not be needed |

### 3.2 Current Files in `generated/table-definitions/`

| File | Content |
|------|---------|
| `Actor.yaml` | MSSQL→PostgreSQL field mapping, types, PK, topic format |
...
├── 0-cdc-merge-control.sql
├── 1-create-customer-schemas.sql
├── ...

# NEW (canonical target)
migrations/                        ← manage-migrations output root
├── infrastructure/                #   Run once per database
│   ├── cdc-merge-control.sql      #     Merge control system
│   └── cdc-processing-log.sql     #     Offset tracking / gap detection
├── schemas/                       #   CREATE SCHEMA per customer/service
│   └── create-schemas.sql         #     Auto-generated from source-groups/service config
├── tables/                        #   CREATE TABLE per CDC table
│   ├── Actor.sql                  #     Uses {{SCHEMA}} placeholder
│   ├── Fraver.sql
│   └── ...                        #     One file per source table
└── staging/                       #   UNLOGGED staging + merge procedures
    ├── Actor-staging.sql
    ├── Fraver-staging.sql
    └── ...                        #     One file per source table
```
...
> **Plan:** [Phase 2 — Service Config Integration](MIGRATION_IMPLEMENTATION_PLAN.md#phase-2-service-config-integration)

### 4.1 Does migration generation consider the current service structure?

**🔴 No.** There is a complete disconnect:
...
adopus:
  source:
    validation_database: AdOpusTest     # ← MSSQL reference for schema inspection
    tables:                             # ← 39 source tables
      dbo.Actor:
        ignore_columns: [Døv, Fødeland] # ← Should exclude columns from migration DDL
      dbo.ActorActor: {}
      # ... 37 more tables
  ...
  sinks:
    sink_asma.directory:
      tables:
        adopus.Actor:                   # ← Sink table config
          target_exists: false          # ← Table needs to be created
          replicate_structure: true     # ← Clone from source schema
          column_templates:
            - template: customer_id     # ← Extra column to add
          from: dbo.Actor               # ← Source table mapping
        public.customer_user:           # ← Different table
          target_exists: true           # ← Table already exists, just map columns
          columns:                      # ← Column rename mapping
...
4. Use `target_exists: false` + `replicate_structure: true` → generate CREATE TABLE DDL
5. Use `target_exists: true` → skip DDL (table already exists in target)
6. Apply `column_templates` → add extra columns to DDL
...
> **Plan:** [Phase 3 — Schema Evolution](MIGRATION_IMPLEMENTATION_PLAN.md#phase-3-schema-evolution)

### 5.1 Current State

**🔴 No versioning exists.** There is:
...
|----------|-----------------|
| New column added to source | ❌ Not detected, not migrated |
| Column type changed in source | ❌ Not detected |
| Column removed from source | ❌ Not detected |
| New table added to service | ❌ Must manually create DDL |
| Primary key changed | ❌ Not detected |
| New column added to `ignore_columns` | ❌ Not detected |
| New customer added | ❌ Must manually edit `1-create-customer-schemas.sql` |

---

## 6. Audit: Staging / Unlogged Table Generation

> **Plan:** [Phase 1E — Staging Table + Merge Procedure](MIGRATION_IMPLEMENTATION_PLAN.md#1e-staging-table--merge-procedure-generation)

### 6.1 Current State

**🟡 Partially implemented but stale.**

The existing staging SQL (`4-*-staging.sql`) is well-designed:

- ✅ UNLOGGED tables (no WAL overhead)
- ✅ `autovacuum_enabled = false` (TRUNCATE after merge, no vacuum)
- ✅ Kafka metadata columns (`__kafka_offset`, `__kafka_partition`, `__kafka_timestamp`)
- ✅ Batch merge procedures with `DISTINCT ON` dedup
- ✅ Processing log integration (offset tracking)
- ✅ TRUNCATE after successful merge

**Problems:**
- ❌ Only 3 tables have staging SQL (39 in service config)
- ❌ Not auto-generated from current config
- ❌ The pipeline generator (`helpers_batch.py`) generates staging INSERT YAML that references `stg_*` tables — but the DDL to create them doesn't exist for most tables
...
> **Plan:** [Phase 1D — Table DDL Generation](MIGRATION_IMPLEMENTATION_PLAN.md#1d-table-ddl-generation) (tasks 1D.1, 1D.2)
...
|-------|---------|----------------------|
| `false` | Table does NOT exist in target → must be created | Migration should generate `CREATE TABLE` DDL |
| `true` | Table already exists → just map columns | Migration should SKIP DDL (or validate schema matches) |

**Current migration handling: 🔴 None.** The stale migration files don't read this flag.
...
| Value | Meaning | Migration Implication |
|-------|---------|----------------------|
| `true` | Clone source structure with type mapping | Read source schema, map types (MSSQL→PG), generate DDL |
...
- ❌ Not wired into migration generation
- ✅ Used by pipeline generation for `replicate_structure` tables
...
> **Plan:** [Phase 1C — Infrastructure SQL](MIGRATION_IMPLEMENTATION_PLAN.md#1c-infrastructure-sql) (centralized in `cdc_management`)

### 8.1 Current State

**🟡 Template exists, not auto-generated per table.**

The `2-cdc-processing-log.sql` creates:
- ✅ `cdc_processing_log` table (tracks offset ranges per table/partition)
- ✅ `v_cdc_processing_gaps` view (detects gaps using LEAD window function)
- ✅ `v_cdc_replication_lag` view (monitors lag per table/partition)
- ✅ `v_cdc_processing_stats` view (batch statistics)

**Gap analysis:** The processing log uses `{{SCHEMA}}` placeholder — it must be applied per customer schema. But the application logic was in the deleted script.

### 8.2 Not Implemented

- ❌ No per-table offset initialization SQL
- ❌ No auto-detection of where to resume after a gap
- ❌ No Redpanda offset reset tooling
- ❌ No integration with `manage-migrations` CLI for gap analysis

---

## 9. Audit: Target Schema Creation

> **Plan:** [Phase 1B — Schema Creation SQL](MIGRATION_IMPLEMENTATION_PLAN.md#1b-schema-creation-sql)

### 9.1 Current State

**🟡 Static file, not auto-generated.**

`1-create-customer-schemas.sql`:
- ✅ Creates 26 customer schemas (one per customer)
- ❌ Hardcoded — does not read from `source-groups.yaml` or service config
- ❌ Adding a new customer requires manual edit + regeneration
- ❌ Does NOT create schemas referenced in sink config (e.g., `public`, `cdc_management`)
...
> **Plan:** [Phase 5 — Pattern Support (db-shared)](MIGRATION_IMPLEMENTATION_PLAN.md#phase-5-pattern-support-db-shared)
...
- `db-per-tenant`: Extract customer list → one schema per customer
- `db-shared`: Extract service list → one schema per service

---

## 11. Best-Practice Recommendations

> **Plan:** Recommendations mapped to plan phases — [Phase 4 (Migration State Tracking)](MIGRATION_IMPLEMENTATION_PLAN.md#phase-4-migration-state-tracking), [Phase 6 (Operational Tooling)](MIGRATION_IMPLEMENTATION_PLAN.md#phase-6-operational-tooling)
...
- Generate corresponding `DOWN` migration (at minimum for `CREATE TABLE → DROP TABLE`)
...
**Issue:** Current migrations only handle the `db-per-tenant` source-side pattern (MSSQL → customer schemas). The sink-side (e.g., `sink_asma.directory`) has different target tables with `target_exists: true` that may need initialization scripts.
...
Priority: **P0 — Blocking all other migration work**
...
| 1.3 | Implement final table DDL generation → `migrations/tables/` | Uses `structure_replicator.py` patterns |
| 1.4 | Implement staging table + merge procedure generation → `migrations/staging/` | Port from stale SQL templates |
| 1.5 | Implement customer schema SQL generation → `migrations/schemas/` | Read customers from source-groups.yaml |
| 1.6a | Generate infrastructure SQL → `migrations/infrastructure/` | Port merge-control + processing-log |
...
Priority: **P0 — Enable existing subcommands**

| Task | Description |
|------|-------------|
| 2.1 | Add `enable-cdc`, `apply-replica`, `clean-cdc` to `MIGRATION_COMMANDS` dict — OR — remove dead Click registrations |
...
Priority: **P1 — Required for asma (db-shared)**
...
| `cdc_generator/cli/schema_docs.py` | Generator | MSSQL introspection → YAML (works) |
...
| `cdc_generator/helpers/type_mapper.py` | Generator | MSSQL → PostgreSQL type mapping |
...
| `generated/pg-migrations/*.sql` | Adopus impl | **STALE** — to be replaced by `migrations/` (3 of 39 tables) |
